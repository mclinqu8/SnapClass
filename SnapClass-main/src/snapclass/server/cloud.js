...
/*
    Modified (4/2022) to run on Node.js backend for SnapClass
*/

// Import modules
const { hex_sha512 } = require('./sha512');
...
Cloud.prototype.request = function (
    ...
    try {
        ...
        var myself = this;
        var rawResponse = wantsRawResponse === 'true';
        axios(options)
            .then((data) => {
                if (data.status === 200) {
                    var response = data.data;
                    onSuccess.call(null, JSON.stringify(response));
                } else {
                    if (onError) {
                        onError.call(
                            null,
